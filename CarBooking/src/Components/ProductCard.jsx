...
export default function ProductCard({ product }) {
  ...
  return (
    <div
      ...
      className="group relative bg-[#050b14]
      border border-sky-400
      shadow-[0_20px_50px_rgba(56,189,248,0.25)]
      rounded-2xl overflow-hidden cursor-pointer
    ...
    >
      ...
      {/* OFFER */}
      {product.offer && (
        <span className="absolute top-0 left-0 z-20 bg-gradient-to-r from-sky-500 to-cyan-400 text-black text-[10px] sm:text-xs font-bold px-2 sm:px-4 py-1 rounded-br-xl">
          {product.offer}% OFF
        </span>
      )}

      {/* IMAGE */}
      <div className="relative h-[140px] sm:h-[180px] md:h-[200px] bg-[#0a0a0b] flex items-center justify-center">
        <img
          src={
            product.images && product.images.length > 0
              ? product.images[0]
          ...
          alt={product.name}
          className="max-h-full max-w-full object-contain scale-105 md:scale-110 transition duration-300 group-hover:scale-110 md:group-hover:scale-115"
          onError={(e) => {
            e.target.onerror = null;
            e.target.src =
              "https://via.placeholder.com/600x400?text=Product+Image";
      ...
      {/* CONTENT */}
      <div className="p-3 sm:p-4 space-y-1 sm:space-y-2 flex flex-col flex-1">
        ...
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-xs sm:text-sm md:text-base font-bold leading-tight text-white">
            {product.name}
          </h3>

          <span
            className={`text-[9px] sm:text-[10px] px-2 py-1 rounded-full whitespace-nowrap
            ${
              product.totalStock > 0
                ? "bg-green-500/20 text-green-400"
                : "bg-red-500/20 text-red-400"
          ...
          >
            {product.totalStock > 0 ? "In" : "Out"}
          </span>
        </div>

        {/* PRICE */}
        <div className="flex items-center gap-2">
          <span className="text-sm sm:text-lg md:text-xl font-extrabold text-white">
            ₹{product.offerPrice}
          </span>

          <span className="line-through text-gray-500 font-bold text-[10px] sm:text-sm">
            ₹{product.mrp}
          </span>
        </div>
